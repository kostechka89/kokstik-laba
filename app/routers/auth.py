...
def issue_tokens(
    ...
    refresh_token, jti, expire_at = create_refresh_token(user.id)
    ttl_seconds = int((expire_at - datetime.utcnow()).total_seconds())
    cache_refresh_session(
        jti,
        {
            "refresh_jti": jti,
...
def refresh_tokens(
    ...
    session = get_refresh_session(jti)
    ...
    replace_refresh_session(jti, new_jti, session, ttl_seconds)
    return schemas.TokenPair(access_token=access_token, refresh_token=refresh_token)
...
def logout(
    ...
    session = get_refresh_session(jti)
    if session:
        remove_refresh_session(jti, user_id)
    return None
